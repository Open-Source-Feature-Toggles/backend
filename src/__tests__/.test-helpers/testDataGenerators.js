...
class FakeUser {
  ...
  async createFakeAccount() {
    if (this.responses.LoginFakeAccountResponse) {
      return;
    }
    const response = await request(this.app).post('/auth/sign-up').send({
  ...
  async LoginFakeAccount() {
    const response = await request(this.app).post('/auth/login').send({
      username: this.username,
      password: this.password,
    });
    this.accessToken = response.body.accessToken;
  ...
  ParseCookie(response_object) {
    let full_cookie = response_object.headers['set-cookie'][0]
      .split(';')[0]
      .split('=');
...
async function makeFeature(args) {
  ...
  let fakeFeature = new FakeFeature(
    app,
    featureName,
    description,
    initialVariableKey,
...
async function makeVariable(args) {
  const { app, fakeUser, fakeProject, fakeFeature, newVariableName } = args;
  let fakeVariable = new FakeVariable(
    app,
    fakeUser,
    fakeProject,
    fakeFeature,
