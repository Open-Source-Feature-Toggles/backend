...
async function SuccessfulSignup() {
  return await request(app).post('/auth/sign-up').send({
    username: USERNAME,
    password: PASSWORD,
...
describe('Signup - Success cases', () => {
  ...
  it('Successfully returns the user an accessToken after signing you up', async () => {
    expect(fakeUser.responses.createFakeAccountResponse.body).toHaveProperty(
      'accessToken',
    );
  });
...
describe('Signup - Error cases', () => {
  ...
  it('Attempts to signup with a username that is already taken', async () => {
    let badAttempt = await unsuccessfulSignup(
      USERNAME,
      PASSWORD,
      CONFIRM_PASSWORD,
    );
