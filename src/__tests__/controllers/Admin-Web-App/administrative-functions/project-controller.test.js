...
beforeEach(async () => {
  let { fakeProject, fakeUser } = await makeUsernandProject(
    app,
    USERNAME,
    PASSWORD,
    PROJECT_NAME,
...
describe('Tests MakeNewProject function in Project Controller', () => {
  describe('MakeNewProject - Success Cases', () => {
    it('Calls MakeNewProject and returns a 200 status', () => {
      let makeProjectResponse =
        testProject.retrieveResponse('CreateFakeProject');
...
describe('Tests DeleteProject function in Project Controller', () => {
  describe('DeleteProject - Success Cases', () => {
    ...
    it('Deletes all features and variables associated with the project', async () => {
       ...
       */
      await clearDatabase();
      let { fakeProject } = await makeUserProjectFeatureandVariable({
        app,
        username: USERNAME,
        password: PASSWORD,
      ...
      let featureBeforeDelete = await Feature.findOne({
        parentProjectName: PROJECT_NAME,
      });
      let variablesBeforeDelete = await Variable.findOne({
        parentFeatureName: 'fake-feature',
      });
      let projectBeforeDelete = await Project.findOne({ name: PROJECT_NAME });
      await fakeProject.DeleteFakeProject();
      let featureAfterDelete = await Feature.findOne({
        parentProjectName: PROJECT_NAME,
      });
      let variablesAfterDelete = await Variable.findOne({
        parentFeatureName: 'fake-feature',
      });
      ...
      expect(
        featureBeforeDelete.variables.includes(variablesBeforeDelete._id),
      ).toEqual(true);
