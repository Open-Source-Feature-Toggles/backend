...
beforeEach(async () => {
  const { fakeUser, fakeProject, fakeFeature } =
...
describe('Tests DeleteFeature function in Feature Controller', () => {
  describe('DeleteFeature - Success Cases', () => {
    ...
    it("Successfully removes feature from the project's feature array", async () => {
       ...
       */
      let featureID = (await Feature.findOne({ name: fakeData.featureName }))
        ._id;
      let projectBeforeDelete = await Project.findOne({
        name: fakeData.projectName,
      });
      let isFeatureIdinProjectArray =
        projectBeforeDelete.features.includes(featureID);
      await feature.DeleteFeature();
      let projectAfterDelete = await Project.findOne({
        name: fakeData.projectName,
      });
      let isFeatureIdNotInProjectArray =
        projectAfterDelete.features.includes(featureID);
    ...
    it("Successfully deletes all of the feature's children variables", async () => {
      ...
      let childVariableBeforeDelete = await Variable.find({
        parentFeatureName: fakeData.featureName,
      });
      await feature.DeleteFeature();
      let childVariableAfterDelete = await Variable.find({
        parentFeatureName: fakeData.featureName,
      });
...
describe('Tests MakeNewFeature function in Feature Controller', () => {
  ...
  describe('MakeNewFeature - Error Cases', () => {
    ...
    it('Tries to create a feature for a nonexistent project', async () => {
       ...
       * this is testing.
       */
      await Promise.all([project.DeleteFakeProject()]);
      let response = await feature.CreateFakeFeature();
      expect(response.status).toBe(404);
    });
  });
