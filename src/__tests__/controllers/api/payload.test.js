...
describe('GetPayload - Success Cases', () => {
  ...
  beforeEach(async () => {
    devPayloadResponse = await requestPayload(
      fakeData.app,
      project.state.developmentApiKey,
    );
    prodPayloadResponse = await requestPayload(
      fakeData.app,
      project.state.productionApiKey,
    );
  });
  ...
  it('Should call GetPayload matching last_updated value and get a 304 status', async () => {
    let callDevAgain = await requestPayload(
      fakeData.app,
      project.state.developmentApiKey,
      devPayloadResponse.body.last_updated,
    );
    let callProdAgain = await requestPayload(
      fakeData.app,
      project.state.productionApiKey,
      prodPayloadResponse.body.last_updated,
    );
  ...
  it('Should call GetPayload and return an accurate payload when the cache is empty', async () => {
     ...
     */
    await flushCache();
    let [devCache, prodCache] = await getProjectsCacheEntries(project);
    let devPayload = (
      await requestPayload(fakeData.app, project.state.developmentApiKey)
    ).body;
    let prodPayload = (
      await requestPayload(fakeData.app, project.state.productionApiKey)
    ).body;
    expect(
      getVariableFromCache(
        devCache,
