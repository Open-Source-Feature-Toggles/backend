...
async function MakeNewVariable(req, res, next) {
  try {
    let { name, parentFeature, projectName } = req.body;
    ...
    await Promise.all([newVariable.save(), findParentFeature.save()]);
...
async function UpdateProductionStatus(req, res, next) {
  try {
    let { name, parentFeature } = req.body;
...
async function UpdateDevelopmentStatus(req, res, next) {
  try {
    ...
    res
      .status(200)
      .json({ developmentEnabled: getVariable.developmentEnabled });
    return next();
  } catch (error) {
    console.error(error);
    res.sendStatus(500);
...
exports.POST_make_new_variable = [MakeNewVariableValidation, MakeNewVariable];

exports.DELETE_delete_variable = [DeleteVariableValidation, DeleteVariable];
