...
async function ChangeProductionStatus(req, res, next) {
  try {
    let { featureName, projectName } = req.body;
    let feature = await FeatureExistsQuery(
      featureName,
      req.user,
      projectName,
    ).exec();
...
exports.POST_delete_feature = [Name_ProjectName_Validation, DeleteFeature];

exports.POST_make_new_feature = [Make_New_Feature_Validation, MakeNewFeature];
